...
public class AudioVisualizer : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < samples.Length; i++)
        {
            ...
            tempCube = (GameObject)Instantiate(cube, new Vector3(goTransform.position.x + i*0.1f, goTransform.position.y, goTransform.position.z), Quaternion.identity);
    ...
    void Update()
    {
        ...
        for (int i = 0; i < samples.Length; i++)
        {
            ...
            {
                if (cubesTransform[i].position.y - gravity[1] > goTransform.position.y)
                cubesTransform[i].position -= gravity;
                else
                 cubesTransform[i].position = new Vector3(cubesTransform[i].position.x, goTransform.position.y, cubesTransform[i].position.z);
                //print("LOOP Gravity working");
